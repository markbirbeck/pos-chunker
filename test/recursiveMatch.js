...
describe('recursiveMatch', function() {
  describe('optional', function() {
    it('should match a symettrical pattern', function() {
      rm
        .recursiveMatch('aaazzz', 'a(?R)?z')[0]
        .should.equal('aaazzz');
    });

    it('should match a symettrical pattern at greater depth', function() {
      rm
        .recursiveMatch('aaaaaazzzzzz', 'a(?R)?z')[0]
        .should.equal('aaaaaazzzzzz');
    });
  });
  ...
  describe('quantifier', function() {
    it('should not match since not enough q\'s', function() {
      rm
        .recursiveMatch('aqqz', 'a(?R){3}z|q')[0]
        .should.equal('q');
    });
    it('should match', function() {
      rm
        .recursiveMatch('aqqqz', 'a(?R){3}z|q')[0]
        .should.equal('aqqqz');
    });

    it('should not match since too many q\'s', function() {
      rm
        .recursiveMatch('aqqqqz', 'a(?R){3}z|q')[0]
        .should.equal('q');
    });

    it('should match nested quantifiers', function() {
      rm
        .recursiveMatch('aqaqqqzqz', 'a(?R){3}z|q')[0]
        .should.equal('aqaqqqzqz');
    });

    it('should match nested quantifiers (long)', function() {
      rm
        .recursiveMatch('aaaqqaqqqzzaqqqzqzqaqqaaqqqzqqzzz', 'a(?R){3}z|q')[0]
        .should.equal('aaaqqaqqqzzaqqqzqzqaqqaaqqqzqqzzz');
    });
  });
  ...
  describe('balanced constructs', function () {
    ...
    it('should match nested parenthesis when using named groups as part of atomic grouping', function() {
      rm
        .recursiveMatch(
          '(a (b (c)))', '\\((?:(?=(?<' + rm.NAMED_GROUP_PLACEHOLDER +
